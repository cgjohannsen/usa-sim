...
class DMS:
    ...
    def schedule(self, task_set):
        ...
        sorted_task_set = sorted(task_set, key=lambda x: x.d, reverse=False)
        ...
        for time_step in range(time_schedule_repeats):
            ...
            for i, t in enumerate(sorted_task_set):
                ...
                # Misses deadline. Is this good enough for exact analysis?
                if time_step % t.d == 0 and ready_arr[i] and time_step > 0:
                    print(str(time_step) + "  " + str(i) + "   UHHHHHHH")
                    return None

                # The task arrives
                if time_step % t.p == 0 and not ready_arr[i]:
                    ready_arr[i] = True
    ...
    def sched_test(self, task_set):
        ...
        for i, t in enumerate(task_set):
            total += t.c + t.d
        if total <= num_tasks * (math.pow(2, 1 / num_tasks) - 1):
            return True
    ...
    def do_exact_analysis(self, sub_task_set, analyzing_task_i):
        ...
        while True:
            ...
            if cur_total == last_total:
                return True
            elif cur_total > sub_task_set[analyzing_task_i].d:
                return False
            else:
                last_total = cur_total
                cur_total = 0
